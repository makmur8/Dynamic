...
static bool InitHTTPAllowList()
{
    ...
    LogPrint(DYNLog::HTTP, "Allowing HTTP connections from: %s\n", strAllowed);
    return true;
}
...
static void http_request_cb(struct evhttp_request* req, void* arg)
{
    ...
    LogPrint(DYNLog::HTTP, "Received a %s request for %s from %s\n",
...
{
    LogPrint(DYNLog::HTTP, "Rejecting request while shutting down\n");
...
static bool ThreadHTTP(struct event_base* base, struct evhttp* http)
{
    ...
    LogPrint(DYNLog::HTTP, "Entering http event loop\n");
    event_base_dispatch(base);
    // Event loop will be interrupted by InterruptHTTPServer()
    LogPrint(DYNLog::HTTP, "Exited http event loop\n");
...
static bool HTTPBindAddresses(struct evhttp* http)
{
    ...
    } else if (IsArgSet("-rpcbind")) { // Specific bind address
    ...
    for (std::vector<std::pair<std::string, uint16_t> >::iterator i = endpoints.begin(); i != endpoints.end(); ++i) {
        LogPrint(DYNLog::HTTP, "Binding RPC on address %s port %i\n", i->first, i->second);
...
# define EVENT_LOG_WARN _EVENT_LOG_WARN
    ...
    if (severity >= EVENT_LOG_WARN) // Log warn messages and higher without debug category
        ...
        LogPrint(DYNLog::LIBEVENT, "libevent: %s\n", msg);
}
...
bool InitHTTPServer()
{
    ...
    LogPrint(DYNLog::HTTP, "Initialized HTTP server\n");
...
{
    LogPrint(DYNLog::HTTP, "Starting HTTP server\n");
...
{
    LogPrint(DYNLog::HTTP, "Interrupting HTTP server\n");
    if (eventHTTP) {
        // Unlisten sockets
...
void StopHTTPServer()
{
    LogPrint(DYNLog::HTTP, "Stopping HTTP server\n");
    if (workQueue) {
        LogPrint(DYNLog::HTTP, "Waiting for HTTP worker threads to exit\n");
#ifndef WIN32
    ...
    if (eventBase) {
        LogPrint(DYNLog::HTTP, "Waiting for HTTP event thread to exit\n");
    ...
    LogPrint(DYNLog::HTTP, "Stopped HTTP server\n");
}
...
{
    LogPrint(DYNLog::HTTP, "Registering HTTP handler for %s (exactmatch %d)\n", prefix, exactMatch);
...
void UnregisterHTTPHandler(const std::string &prefix, bool exactMatch)
{
    ...
    if (i != iend)
    {
        LogPrint(DYNLog::HTTP, "Unregistering HTTP handler for %s (exactmatch %d)\n", prefix, exactMatch);
