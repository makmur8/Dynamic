...
bool CDynodeMan::Add(CDynode &dn)
{
    ...
    if (pdn == NULL) {
        LogPrint(DYNLog::DYNODE, "CDynodeMan::Add -- Adding new Dynode: addr=%s, %i now\n", dn.addr.ToString(), size() + 1);
...
void CDynodeMan::Check()
{
    ...
    LogPrint(DYNLog::DYNODE, "CDynodeMan::Check -- nLastWatchdogVoteTime=%d, IsWatchdogActive()=%d\n", nLastWatchdogVoteTime, IsWatchdogActive());
...

    {
        ...
        while(it != vDynodes.end()) {
            ...
            if ((*it).IsOutpointSpent()) {
                LogPrint(DYNLog::DYNODE, "CDynodeMan::CheckAndRemove -- Removing Dynode: %s  addr=%s  %i now\n", (*it).GetStateString(), (*it).addr.ToString(), size() - 1);
                ...
                if(fAsk) {
                    ...
                    if(fAskedForDnbRecovery) {
                        LogPrint(DYNLog::DYNODE, "CDynodeMan::CheckAndRemove -- Recovery initiated, Dynode=%s\n", it->vin.prevout.ToStringShort());
                        nAskForDnbRecovery--;
                    }
        ...
        LogPrint(DYNLog::DYNODE, "CDynodeMan::CheckAndRemove -- mDnbRecoveryGoodReplies size=%d\n", (int)mDnbRecoveryGoodReplies.size());
        ...
        while(itDnbReplies != mDnbRecoveryGoodReplies.end()){
            if(mDnbRecoveryRequests[itDnbReplies->first].first < GetTime()) {
                ...
                if(itDnbReplies->second.size() >= DNB_RECOVERY_QUORUM_REQUIRED) {
                    ...
                    LogPrint(DYNLog::DYNODE, "CDynodeMan::CheckAndRemove -- reprocessing dnb, Dynode=%s\n", itDnbReplies->second[0].vin.prevout.ToStringShort());
                ...
                LogPrint(DYNLog::DYNODE, "CDynodeMan::CheckAndRemove -- removing dnb recovery reply, Dynode=%s, size=%d\n", itDnbReplies->second[0].vin.prevout.ToStringShort(), (int)itDnbReplies->second.size());
    ...
    }
    {
        ...
        while(it4 != mapSeenDynodePing.end()){
            if((*it4).second.IsExpired()) {
                LogPrint(DYNLog::DYNODE, "CDynodeMan::CheckAndRemove -- Removing expired Dynode ping: hash=%s\n", (*it4).second.GetHash().ToString());
        ...
        while(itv2 != mapSeenDynodeVerification.end()){
            if((*itv2).second.nBlockHeight < pCurrentBlockIndex->nHeight - MAX_POSE_BLOCKS){
                LogPrint(DYNLog::DYNODE, "CDynodeMan::CheckAndRemove -- Removing expired Dynode verification: hash=%s\n", (*itv2).first.ToString());
...
void CDynodeMan::SsegUpdate(CNode* pnode)
{
    ...
    LogPrint(DYNLog::DYNODE, "CDynodeMan::SsegUpdate -- asked %s for the list\n", pnode->addr.ToString());
}
...
CDynode* CDynodeMan::FindRandomNotInVec(const std::vector<CTxIn> &vecToExclude, int nProtocolVersion)
{
    ...
    BOOST_FOREACH(CDynode* pdn, vpDynodesShuffled) {
        ...
        LogPrint(DYNLog::DYNODE, "CDynodeMan::FindRandomNotInVec -- found, Dynode=%s\n", pdn->vin.prevout.ToStringShort());
        return pdn;
    }

    LogPrint(DYNLog::DYNODE, "CDynodeMan::FindRandomNotInVec -- failed\n");
    return NULL;
}
...
void CDynodeMan::ProcessMessage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == NetMsgType::DNANNOUNCE) { //Dynode Broadcast
        ...
        LogPrint(DYNLog::DYNODE, "DNANNOUNCE -- Dynode announce, Dynode=%s\n", dnb.vin.prevout.ToStringShort());
        ...
        LogPrint(DYNLog::DYNODE, "DNPING -- Dynode ping, Dynode=%s\n", dnp.vin.prevout.ToStringShort());
        ...
        LogPrint(DYNLog::DYNODE, "DNPING -- Dynode ping, Dynode=%s new\n", dnp.vin.prevout.ToStringShort());
        ...
        LogPrint(DYNLog::DYNODE, "SSEG -- Dynode list, Dynode=%s\n", vin.prevout.ToStringShort());
        ...
        BOOST_FOREACH(CDynode& dn, vDynodes) {
            ...
            LogPrint(DYNLog::DYNODE, "SSEG -- Sending Dynode entry: Dynode=%s  addr=%s\n", dn.vin.prevout.ToStringShort(), dn.addr.ToString());
        ...
        LogPrint(DYNLog::DYNODE, "SSEG -- No invs sent to peer %d\n", pfrom->id);
...
void CDynodeMan::DoFullVerificationStep()
{
    ...
    while(it != vecDynodeRanks.end()) {
        if(it->first > MAX_POSE_RANK) {
            LogPrint(DYNLog::DYNODE, "CDynodeMan::DoFullVerificationStep -- Must be in top %d to send verify request\n",
                        (int)MAX_POSE_RANK);
            return;
        }
        if(it->second.vin == activeDynode.vin) {
            nMyRank = it->first;
            LogPrint(DYNLog::DYNODE, "CDynodeMan::DoFullVerificationStep -- Found self at rank %d/%d, verifying up to %d Dynodes\n",
    ...
    while(it != vecDynodeRanks.end()) {
        if(it->second.IsPoSeVerified() || it->second.IsPoSeBanned()) {
            LogPrint(DYNLog::DYNODE, "CDynodeMan::DoFullVerificationStep -- Already %s%s%s Dynode %s address %s, skipping...\n",
        ...
        LogPrint(DYNLog::DYNODE, "CDynodeMan::DoFullVerificationStep -- Verifying Dynode %s rank %d/%d address %s\n",
    ...
    LogPrint(DYNLog::DYNODE, "CDynodeMan::DoFullVerificationStep -- Sent verification requests to %d Dynodes\n", nCount);
}
...
bool CDynodeMan::SendVerifyRequest(const CAddress& addr, const std::vector<CDynode*>& vSortedByAddr)
{
    if(netfulfilledman.HasFulfilledRequest(addr, strprintf("%s", NetMsgType::DNVERIFY)+"-request")) {
        ...
        LogPrint(DYNLog::DYNODE, "CDynodeMan::SendVerifyRequest -- too many requests, skipping... addr=%s\n", addr.ToString());
        return false;
    }
...
void CDynodeMan::ProcessVerifyReply(CNode* pnode, CDynodeVerification& dnv)
{
...

    {
        ...
        BOOST_FOREACH(CDynode* pdn, vpDynodesToBan) {
            pdn->IncreasePoSeBanScore();
            LogPrint(DYNLog::DYNODE, "CDynodeMan::ProcessVerifyBroadcast -- increased PoSe ban score for %s addr %s, new score %d\n",
...
void CDynodeMan::ProcessVerifyBroadcast(CNode* pnode, const CDynodeVerification& dnv)
{
    ...
    if(dnv.nBlockHeight < pCurrentBlockIndex->nHeight - MAX_POSE_BLOCKS) {
        LogPrint(DYNLog::DYNODE, "DynodeMan::ProcessVerifyBroadcast -- Outdated: current block %d, verification block %d, peer=%d\n",
                    pCurrentBlockIndex->nHeight, dnv.nBlockHeight, pnode->id);
        return;
    }

    if(dnv.vin1.prevout == dnv.vin2.prevout) {
        LogPrint(DYNLog::DYNODE, "DynodeMan::ProcessVerifyBroadcast -- ERROR: same vins %s, peer=%d\n",
    ...
    if(nRank < MAX_POSE_RANK) {
        LogPrint(DYNLog::DYNODE, "DynodeMan::ProcessVerifyBroadcast -- Dynode is not in top %d, current rank %d, peer=%d\n",
...
bool CDynodeMan::CheckDnbAndUpdateDynodeList(CNode* pfrom, CDynodeBroadcast dnb, int& nDos)
{
    ...
    nDos = 0;
    LogPrint(DYNLog::DYNODE, "CDynodeMan::CheckDnbAndUpdateDynodeList -- Dynode=%s\n", dnb.vin.prevout.ToStringShort());

    uint256 hash = dnb.GetHash();
    if(mapSeenDynodeBroadcast.count(hash) && !dnb.fRecovery) { //seen      
        LogPrint(DYNLog::DYNODE, "CDynodeMan::CheckDnbAndUpdateDynodeList -- Dynode=%s seen\n", dnb.vin.prevout.ToStringShort());
        ...
        if(GetTime() - mapSeenDynodeBroadcast[hash].first > DYNODE_NEW_START_REQUIRED_SECONDS - DYNODE_MIN_DNP_SECONDS * 2) {
            LogPrint(DYNLog::DYNODE, "CDynodeMan::CheckDnbAndUpdateDynodeList -- Dynode=%s seen update\n", dnb.vin.prevout.ToStringShort());
        ...
        if(pfrom && IsDnbRecoveryRequested(hash) && GetTime() < mDnbRecoveryRequests[hash].first) {
            LogPrint(DYNLog::DYNODE, "CDynodeMan::CheckDnbAndUpdateDynodeList -- dnb=%s seen request\n", hash.ToString());
            if(mDnbRecoveryRequests[hash].second.count(pfrom->addr)) {
                LogPrint(DYNLog::DYNODE, "CDynodeMan::CheckDnbAndUpdateDynodeList -- dnb=%s seen request, addr=%s\n", hash.ToString(), pfrom->addr.ToString());
                ...
                if(dnb.lastPing.sigTime > mapSeenDynodeBroadcast[hash].second.lastPing.sigTime) {
                    ...
                    dnTemp.Check();
                    LogPrint(DYNLog::DYNODE, "CDynodeMan::CheckDnbAndUpdateDynodeList -- dnb=%s seen request, addr=%s, better lastPing: %d min ago, projected dn state: %s\n", hash.ToString(), pfrom->addr.ToString(), (GetTime() - dnb.lastPing.sigTime)/60, dnTemp.GetStateString());
                    if(dnTemp.IsValidStateForAutoStart(dnTemp.nActiveState)) {
                        // this node thinks it's a good one
                        LogPrint(DYNLog::DYNODE, "CDynodeMan::CheckDnbAndUpdateDynodeList -- Dynode=%s seen good\n", dnb.vin.prevout.ToStringShort());
    ...
    LogPrint(DYNLog::DYNODE, "CDynodeMan::CheckDnbAndUpdateDynodeList -- Dynode=%s new\n", dnb.vin.prevout.ToStringShort());

    if(!dnb.SimpleCheck(nDos)) {
        LogPrint(DYNLog::DYNODE, "CDynodeMan::CheckDnbAndUpdateDynodeList -- SimpleCheck() failed, Dynode=%s\n", dnb.vin.prevout.ToStringShort());
        return false;
    }
    ...
    if(pdn) {
        ...
        if(!dnb.Update(pdn, nDos)) {
            LogPrint(DYNLog::DYNODE, "CDynodeMan::CheckDnbAndUpdateDynodeList -- Update() failed, Dynode=%s\n", dnb.vin.prevout.ToStringShort());
            return false;
        }
...
void CDynodeMan::UpdatedBlockTip(const CBlockIndex *pindex)
{
    ...
    LogPrint(DYNLog::DYNODE, "CDynodeMan::UpdatedBlockTip -- pCurrentBlockIndex->nHeight=%d\n", pCurrentBlockIndex->nHeight);
