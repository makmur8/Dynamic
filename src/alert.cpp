...
bool CAlert::ProcessAlert(const std::vector<unsigned char>& alertKey, bool fThread)
{
...

    {
        ...
        for (std::map<uint256, CAlert>::iterator mi = mapAlerts.begin(); mi != mapAlerts.end();)
        {
            ...
            if (Cancels(alert))
            {
                LogPrint(DYNLog::ALERT, "cancelling alert %d\n", alert.nID);
                uiInterface.NotifyAlertChanged((*mi).first, CT_DELETED);
                mapAlerts.erase(mi++);
            }
            else if (!alert.IsInEffect())
            {
                LogPrint(DYNLog::ALERT, "expiring alert %d\n", alert.nID);
                uiInterface.NotifyAlertChanged((*mi).first, CT_DELETED);
                mapAlerts.erase(mi++);
            }
            else
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CAlert)& item, mapAlerts)
        {
            ...
            {
                LogPrint(DYNLog::ALERT, "alert already cancelled by %d\n", alert.nID);
                return false;
            }
        }
    ...
    LogPrint(DYNLog::ALERT, "accepted alert %d, AppliesToMe()=%d\n", nID, AppliesToMe());
    return true;
}
