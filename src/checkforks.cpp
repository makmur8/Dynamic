...
bool CheckForkIsTrue(ForkID identifier, const CBlockIndex* pindexLast, bool fTableFlip) {
    ...
    if (identifier == DELTA_RETARGET || identifier == PRE_DELTA_RETARGET || identifier == START_DYNODE_PAYMENTS || identifier == FORK_SLOT_3 || identifier == FORK_SLOT_4) {
        ...
        if((pindexLast->nHeight + 1) > consensusParams.nUpdateDiffAlgoHeight && identifier == DELTA_RETARGET) {
            booleanParam = true;
        }
        // Are we using the reward system before DELTA Retargeting's Fork?
        else if (chainActive.Height() < consensusParams.nUpdateDiffAlgoHeight && identifier == PRE_DELTA_RETARGET) {
            booleanParam = true;
        }
        // Have we now formally enabled Dynode Payments?
        else if (chainActive.Height() > consensusParams.nDynodePaymentsStartBlock && identifier == START_DYNODE_PAYMENTS) {
            booleanParam = true;
        }
        // Empty Forking Slot III
        else if (placeholderIntX == placeholderIntY && identifier == FORK_SLOT_3) {
            booleanParam = true;
        }
        // Empty Forking Slot IV
        else if (placeholderIntX == placeholderIntY && identifier == FORK_SLOT_4) {
            booleanParam = true;
        }
        ...
        throw std::runtime_error(strprintf("%s: Unknown Fork Verification Cause! %s.", __func__, identifier));
    }
