...
//    evhttp_add_header(output_headers, "Content-Length", itostr(strMsg.size()).c_str());
    ...
    //LogPrint(DYNLog::KEEPASS, "CKeePassIntegrator::doHTTPPost -- send POST data: %s\n", strPost);
    LogPrint(DYNLog::KEEPASS, "CKeePassIntegrator::doHTTPPost -- send POST data\n");
...
//    LogPrint(DYNLog::KEEPASS, "CKeePassIntegrator::doHTTPPost -- request written\n");
...
//    LogPrint(DYNLog::KEEPASS, "CKeePassIntegrator::doHTTPPost -- request status line read\n");
...
//    LogPrint(DYNLog::KEEPASS, "CKeePassIntegrator::doHTTPPost -- reading response body start\n");
...
//        }
//    }
//    LogPrint(DYNLog::KEEPASS, "CKeePassIntegrator::doHTTPPost -- reading response body end\n");
//
//    // Receive HTTP reply message headers and body
//    std::map<std::string, std::string> mapHeaders;
//    ReadHTTPMessage(response_stream, mapHeaders, strResponse, nProto, std::numeric_limits<size_t>::max());
//    LogPrint(DYNLog::KEEPASS, "CKeePassIntegrator::doHTTPPost -- Processed body\n");
...
void CKeePassIntegrator::rpcTestAssociation(bool bTriggerUnlock)
{
    ...
    LogPrint(DYNLog::KEEPASS, "CKeePassIntegrator::rpcTestAssociation -- send result: status: %d response: %s\n", nStatus, strResponse);
}
...
std::vector<CKeePassIntegrator::CKeePassEntry> CKeePassIntegrator::rpcGetLogins()
{
    ...
    //LogPrint(DYNLog::KEEPASS, "CKeePassIntegrator::rpcGetLogins -- send result: status: %d response: %s\n", nStatus, strResponse);
    LogPrint(DYNLog::KEEPASS, "CKeePassIntegrator::rpcGetLogins -- send result: status: %d\n", nStatus);
...
void CKeePassIntegrator::rpcSetLogin(const SecureString& sWalletPass, const SecureString& sEntryId)
{
    ...
    LogPrint(DYNLog::KEEPASS, "CKeePassIntegrator::rpcSetLogin -- send Url: %s\n", sUrl);
    ...
    LogPrint(DYNLog::KEEPASS, "CKeePassIntegrator::rpcSetLogin -- send result: status: %d response: %s\n", nStatus, strResponse);
...
void CKeePassIntegrator::rpcAssociate(std::string& strId, SecureString& sKeyBase64)
{
    ...
    LogPrint(DYNLog::KEEPASS, "CKeePassIntegrator::rpcAssociate -- send result: status: %d response: %s\n", nStatus, strResponse);
