...
uint16_t DynDns::HandleQuery() {
    ...
    *--key_end = 0; // Remove last dot, set EOLN
    ...
    uint16_t qtype  = *m_rcv++;
    qtype  = (qtype  << 8) + *m_rcv++;
    uint16_t qclass = *m_rcv++;
    qclass = (qclass << 8) + *m_rcv++;
...
void DynDns::Answer_ALL(uint16_t qtype, char *buf) {
    ...
    for(int tok_no = 0; tok_no < tokQty; tok_no++) {
        ...
        switch(qtype) {
        case 1 :
            Fill_RD_IP(tokens[tok_no], AF_INET);
            break;
        case 28:
            Fill_RD_IP(tokens[tok_no], AF_INET6);
            break;
        case 2 :
        case 5 :
        case 12:
            Fill_RD_DName(tokens[tok_no], 0, 0);
            break; // NS,CNAME,PTR
        case 15:
            Fill_RD_DName(tokens[tok_no], 2, 0);
            break; // MX
        case 16:
            Fill_RD_DName(tokens[tok_no], 0, 1);
            break; // TXT
        default:
            break;
        } // swithc
