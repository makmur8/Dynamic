...
{
    LogPrint(DYNLog::GOBJECT, "CGovernanceTriggerManager::CleanAndRemove -- Start\n");
    ...
    LogPrint(DYNLog::GOBJECT, "CGovernanceTriggerManager::CleanAndRemove -- mapTrigger.size() = %d\n", mapTrigger.size());
    ...
    while(it != mapTrigger.end()) {
        ...
        if(!pSuperblock) {
            ...
            LogPrint(DYNLog::GOBJECT, "CGovernanceTriggerManager::CleanAndRemove -- NULL superblock marked for removal\n");
            remove = true;
        } else {
            DBG( cout << "CGovernanceTriggerManager::CleanAndRemove: superblock status = " << pSuperblock->GetStatus() << endl; );
            LogPrint(DYNLog::GOBJECT, "CGovernanceTriggerManager::CleanAndRemove -- superblock status = %d\n", pSuperblock->GetStatus());
            switch(pSuperblock->GetStatus()) {
            case SEEN_OBJECT_ERROR_INVALID:
            case SEEN_OBJECT_UNKNOWN:
                LogPrint(DYNLog::GOBJECT, "CGovernanceTriggerManager::CleanAndRemove -- Unknown or invalid trigger found\n");
                remove = true;
                break;
            ...
            case SEEN_OBJECT_EXECUTED:
            {
                ...
                LogPrint(DYNLog::GOBJECT, "CGovernanceTriggerManager::CleanAndRemove -- nTriggerBlock = %d, nExpirationBlock = %d\n", nTriggerBlock, nExpirationBlock);
                if(governance.GetCachedBlockHeight() > nExpirationBlock) {
                    LogPrint(DYNLog::GOBJECT, "CGovernanceTriggerManager::CleanAndRemove -- Outdated trigger found\n");
                    remove = true;
                    CGovernanceObject* pgovobj = pSuperblock->GetGovernanceObject();
                    if(pgovobj) {
                        LogPrint(DYNLog::GOBJECT, "CGovernanceTriggerManager::CleanAndRemove -- Expiring outdated object: %s\n", pgovobj->GetHash().ToString());
        ...
        if(remove) {
            ...
            LogPrint(DYNLog::GOBJECT, "CGovernanceTriggerManager::CleanAndRemove -- Removing trigger object\n");
            mapTrigger.erase(it++);
        }
        else  {
            ++it;
...
{
    LogPrint(DYNLog::GOBJECT, "CSuperblockManager::IsSuperblockTriggered -- Start nBlockHeight = %d\n", nBlockHeight);
    ...
    LogPrint(DYNLog::GOBJECT, "CSuperblockManager::IsSuperblockTriggered -- vecTriggers.size() = %d\n", vecTriggers.size());
    ...
    BOOST_FOREACH(CSuperblock_sptr pSuperblock, vecTriggers)
    {
        ...
        LogPrint(DYNLog::GOBJECT, "CSuperblockManager::IsSuperblockTriggered -- data = %s\n", pObj->GetDataAsString());

        if(nBlockHeight != Params().GetConsensus().nSuperblockCycle) {
            LogPrint(DYNLog::GOBJECT, "CSuperblockManager::IsSuperblockTriggered -- block height doesn't match nBlockHeight = %d, blockStart = %d, continuing\n",
                     nBlockHeight,
        ...
        if(pObj->IsSetCachedFunding()) {
            LogPrint(DYNLog::GOBJECT, "CSuperblockManager::IsSuperblockTriggered -- fCacheFunding = true, returning true\n");
            DBG( cout << "IsSuperblockTriggered returning true" << endl; );
            return true;
        }
        else  {
            LogPrint(DYNLog::GOBJECT, "CSuperblockManager::IsSuperblockTriggered -- fCacheFunding = false, continuing\n");
...
void CSuperblockManager::CreateSuperblock(CMutableTransaction& txNewRet, int nBlockHeight, std::vector<CTxOut>& voutSuperblockRet)
{
    ...
    if(!CSuperblockManager::GetBestSuperblock(pSuperblock, nBlockHeight)) {
        LogPrint(DYNLog::GOBJECT, "CSuperblockManager::CreateSuperblock -- Can't find superblock for height %d\n", nBlockHeight);
      ...
      vecPayments()
{
    ...
    LogPrint(DYNLog::GOBJECT, "CSuperblock -- nEpochStart = %d, strAddresses = %s, strAmounts = %s, vecPayments.size() = %d\n",
...
CAmount CSuperblock::GetPaymentsLimit(int nBlockHeight)
{
    ...
    LogPrint(DYNLog::GOBJECT, "CSuperblock::GetPaymentsLimit -- Valid superblock height %d, payments max %lld\n", nBlockHeight, nPaymentsLimit);
...
bool CSuperblock::IsValid(const CTransaction& txNew, int nBlockHeight, CAmount blockReward)
{
    ...
    LogPrint(DYNLog::GOBJECT, "CSuperblock::IsValid nOutputs = %d, nPayments = %d, strData = %s\n",
...
std::string CSuperblockManager::GetRequiredPaymentsString(int nBlockHeight)
{
    ...
    if(!GetBestSuperblock(pSuperblock, nBlockHeight)) {
        LogPrint(DYNLog::GOBJECT, "CSuperblockManager::GetRequiredPaymentsString -- Can't find superblock for height %d\n", nBlockHeight);
        return "error";
    }
