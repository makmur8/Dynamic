   ...
   LogPrint(DYNLog::DYNODE, "CDynode::Check -- Dynode %s is in %s state\n", vin.prevout.ToStringShort(), GetStateString());
    ...
    if(fRequireUpdate) {
        ...
        if(nActiveStatePrev != nActiveState) {
            LogPrint(DYNLog::DYNODE, "CDynode::Check -- Dynode %s is in %s state now\n", vin.prevout.ToStringShort(), GetStateString());
        }
        return;
    }
    ...
    if(fWaitForPing && !fOurDynode) {
        ...
        if(IsExpired() || IsWatchdogExpired() || IsNewStartRequired()) {
            LogPrint(DYNLog::DYNODE, "CDynode::Check -- Dynode %s is in %s state, waiting for ping\n", vin.prevout.ToStringShort(), GetStateString());
            return;
        }
    }
    ...
    if(!fWaitForPing || fOurDynode) {
        ...
        LogPrint(DYNLog::DYNODE, "CDynode::Check -- outpoint=%s, nTimeLastWatchdogVote=%d, GetTime()=%d, fWatchdogExpired=%d\n",
...
void CDynode::UpdateLastPaid(const CBlockIndex *pindex, int nMaxBlocksToScanBack)
{
    ...
    // LogPrint(DYNLog::DYNODE, "CDynode::UpdateLastPaidBlock -- searching for block with payment to %s\n", vin.prevout.ToStringShort());
    ...
    // LogPrint(DYNLog::DYNODE, "CDynode::UpdateLastPaidBlock -- searching for block with payment to %s -- keeping old %d\n", vin.prevout.ToStringShort(), nBlockLastPaid);
}
...
bool CDynodeBroadcast::Create(CTxIn txin, CService service, CKey keyCollateralAddressNew, CPubKey pubKeyCollateralAddressNew, CKey keyDynodeNew, CPubKey pubKeyDynodeNew, std::string &strErrorRet, CDynodeBroadcast &dnbRet)
{
    ...
    LogPrint(DYNLog::DYNODE, "CDynodeBroadcast::Create -- pubKeyCollateralAddressNew = %s, pubKeyDynodeNew.GetID() = %s\n",
...
bool CDynodeBroadcast::CheckOutpoint(int& nDos)
{
...

    {
        ...
        if(!lockMain) {
            ...
            LogPrint(DYNLog::DYNODE, "CDynodeBroadcast::CheckOutpoint -- Failed to aquire lock, addr=%s", addr.ToString());
        ...
        if(coins.vout[vin.prevout.n].nValue != 1000 * COIN) {
            LogPrint(DYNLog::DYNODE, "CDynodeBroadcast::CheckOutpoint -- Dynode UTXO should have 1000 DYN, Dynode=%s\n", vin.prevout.ToStringShort());
            return false;
        }
    ...
    LogPrint(DYNLog::DYNODE, "CDynodeBroadcast::CheckOutpoint -- Dynode UTXO verified\n");
...
bool CDynodePing::SimpleCheck(int& nDos)
{
...

    {
        ...
        if (mi == mapBlockIndex.end()) {
            LogPrint(DYNLog::DYNODE, "DynodePing::SimpleCheck -- Dynode ping is invalid, unknown block hash: Dynode=%s blockHash=%s\n", vin.prevout.ToStringShort(), blockHash.ToString());
 ...
 bool CDynodePing::CheckAndUpdate(CDynode* pdn, bool fFromNewBroadcast, int& nDos)
 {
    ...
    if (pdn == NULL) {
        LogPrint(DYNLog::DYNODE, "CDynodePing::CheckAndUpdate -- Couldn't find Dynode entry, Dynode=%s\n", vin.prevout.ToStringShort());
        return false;
    }

    if(!fFromNewBroadcast) {
        if (pdn->IsUpdateRequired()) {
            LogPrint(DYNLog::DYNODE, "CDynodePing::CheckAndUpdate -- Dynode protocol is outdated, Dynode=%s\n", vin.prevout.ToStringShort());
            return false;
        }
    ...
    if (pdn->IsPingedWithin(DYNODE_MIN_DNP_SECONDS - 60, sigTime)) {
        LogPrint(DYNLog::DYNODE, "CDynodePing::CheckAndUpdate -- Dynode ping arrived too early, Dynode=%s\n", vin.prevout.ToStringShort());
    ...
    if(!dynodeSync.IsDynodeListSynced() && !pdn->IsPingedWithin(DYNODE_EXPIRATION_SECONDS/2)) {
        ...
        LogPrint(DYNLog::DYNODE, "CDynodePing::CheckAndUpdate -- bumping sync timeout, dynode=%s\n", vin.prevout.ToStringShort());
        dynodeSync.AddedDynodeList();
    }

    // let's store this ping as the last one
    LogPrint(DYNLog::DYNODE, "CDynodePing::CheckAndUpdate -- Dynode ping accepted, Dynode=%s\n", vin.prevout.ToStringShort());
    pdn->lastPing = *this;
    ...
    LogPrint(DYNLog::DYNODE, "CDynodePing::CheckAndUpdate -- Dynode ping acceepted and relayed, Dynode=%s\n", vin.prevout.ToStringShort());
    Relay();
