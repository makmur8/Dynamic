...
int CWallet::GetRealInputPrivateSendRounds(CTxIn txin, int nRounds) const
{
    ...
    if(wtx != NULL)
    {
        ...
        if (mdwi == mDenomWtxes.end()) {
            ...
            LogPrint(DYNLog::PRIVATESEND, "GetRealInputPrivateSendRounds INSERTING %s\n", hash.ToString());
        ...
        if (nout >= wtx->vout.size()) {
            ...
            LogPrint(DYNLog::PRIVATESEND, "GetRealInputPrivateSendRounds UPDATED   %s %3d %3d\n", hash.ToString(), nout, -4);
            return -4;
        }
...
void CWallet::GenerateNewHDChain()
{
    ...
    // clean up
    DeleteArgument("-hdseed");
    DeleteArgument("-mnemonic");
    DeleteArgument("-mnemonicpassphrase");
}
                                 ...
                                 std::set<std::pair<const CWalletTx*,unsigned int> >& setCoinsRet, CAmount& nValueRet) const
{
    ...
    {
        LogPrint(DYNLog::SELECTCOINS, "tryDenom: %d\n", tryDenom);
        vValue.clear();
        nTotalLower = 0;
...
//            if (fDebug) LogPrint(DYNLog::SELECTCOINS, "value %s confirms %d\n", FormatMoney(pcoin->vout[output.i].nValue), output.nDepth);
            ...
            ((nBest != nTargetValue && nBest < nTargetValue + MIN_CHANGE) || coinLowestLarger.first <= nBest))
    {
        ...
        LogPrint(DYNLog::SELECTCOINS, "%s - total %s\n", s, FormatMoney(nBest));
    }
...
bool CWallet::SelectCoinsGrouppedByAddresses(std::vector<CompactTallyItem>& vecTallyRet, bool fSkipDenominated, bool fAnonymizable) const
{
    ...
    if(fAnonymizable) {
        if(fSkipDenominated && fAnonymizableTallyCachedNonDenom) {
            ...
            LogPrint(DYNLog::SELECTCOINS, "SelectCoinsGrouppedByAddresses - using cache for non-denom inputs\n");
            return vecTallyRet.size() > 0;
        }
        if(!fSkipDenominated && fAnonymizableTallyCached) {
            vecTallyRet = vecAnonymizableTallyCached;
            LogPrint(DYNLog::SELECTCOINS, "SelectCoinsGrouppedByAddresses - using cache for all inputs\n");
            return vecTallyRet.size() > 0;
        }
    }
    ...
    LogPrint(DYNLog::SELECTCOINS, "%s", strMessage);
