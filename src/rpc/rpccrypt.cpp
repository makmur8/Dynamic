...
UniValue encryptdata(const UniValue& params, bool fHelp)
{
...
UniValue decryptdata(const UniValue& params, bool fHelp)
{
    ...
    CKey key;
    CDynamicAddress addr(params[0].get_str());
    if (addr.IsValid()) {
        CKeyID keyID;
        addr.GetKeyID(keyID);
...
UniValue encryptmessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
...
UniValue decryptmessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
