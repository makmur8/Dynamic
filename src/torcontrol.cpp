        ...
        if (ch == ' ') {
            ...
            if (self->message.code >= 600) {
                ...
                if (!self->reply_handlers.empty()) {
                    ...
                    LogPrint(DYNLog::TOR, "tor: Received unexpected sync reply %i\n", self->message.code);
                }
            }
            self->message.Clear();
        }
...
void TorControlConnection::eventcb(struct bufferevent *bev, short what, void *ctx)
{
    ...
    if (what & BEV_EVENT_CONNECTED) {
        LogPrint(DYNLog::TOR, "tor: Successfully connected!\n");
        self->connected(*self);
    } else if (what & (BEV_EVENT_EOF|BEV_EVENT_ERROR)) {
        if (what & BEV_EVENT_ERROR) {
            LogPrint(DYNLog::TOR, "tor: Error connecting to Tor control socket\n");
        } else {
            LogPrint(DYNLog::TOR, "tor: End of stream\n");
        }
        self->Disconnect();
    ...
    reconnect_timeout(RECONNECT_TIMEOUT_START)
{
    ...
    if (pkf.first) {
        LogPrint(DYNLog::TOR, "tor: Reading cached private key from %s\n", GetPrivateKeyFile().string());
...
{
    if (reply.code == 250) {
        LogPrint(DYNLog::TOR, "tor: ADD_ONION successful\n");
...
{
    if (reply.code == 250) {
        LogPrint(DYNLog::TOR, "tor: Authentication successful\n");
...
{
    if (reply.code == 250) {
        LogPrint(DYNLog::TOR, "tor: SAFECOOKIE authentication challenge successful\n");
        ...
        if (l.first == "AUTHCHALLENGE") {
            ...
            LogPrint(DYNLog::TOR, "tor: AUTHCHALLENGE ServerHash %s ServerNonce %s\n", HexStr(serverHash), HexStr(serverNonce));
    ...
    if (reply.code == 250) {
        ...
        BOOST_FOREACH(const std::string &s, reply.lines) {
            ...
            if (l.first == "AUTH") {
                ...
                if ((i = m.find("Tor")) != m.end()) {
                    LogPrint(DYNLog::TOR, "tor: Connected to Tor version %s\n", i->second);
                }
            }
        }
        BOOST_FOREACH(const std::string &s, methods) {
            LogPrint(DYNLog::TOR, "tor: Supported authentication method: %s\n", s);
        }
        ...
        if (!torpassword.empty()) {
            if (methods.count("HASHEDPASSWORD")) {
                LogPrint(DYNLog::TOR, "tor: Using HASHEDPASSWORD authentication\n");
            ...
            LogPrint(DYNLog::TOR, "tor: Using NULL authentication\n");
            _conn.Command("AUTHENTICATE", boost::bind(&TorController::auth_cb, this, _1, _2));
        } else if (methods.count("SAFECOOKIE")) {
            // Cookie: hexdump -e '32/1 "%02x""\n"'  ~/.tor/control_auth_cookie
            LogPrint(DYNLog::TOR, "tor: Using SAFECOOKIE authentication, reading cookie authentication from %s\n", cookiefile);
...
void TorController::disconnected_cb(TorControlConnection& _conn)
{
    ...
    LogPrint(DYNLog::TOR, "tor: Not connected to Tor control port %s, trying to reconnect\n", target);
