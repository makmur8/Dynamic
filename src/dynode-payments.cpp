...
bool IsBlockValueValid(const CBlock& block, int nBlockHeight, CAmount blockReward,std::string &strErrorRet)
{
    ...
    const Consensus::Params& consensusParams = Params().GetConsensus();
     if(nBlockHeight < consensusParams.nSuperblockStartBlock) {
        ...
        if(nBlockHeight >= consensusParams.nBudgetPaymentsStartBlock &&
            ...
            LogPrint(DYNLog::GOBJECT, "IsBlockValueValid -- WARNING: Skipping budget block value checks, accepting block\n");
            // TODO: reprocess blocks to make sure they are legit?
            return true;
        }
        // LogPrint(DYNLog::GOBJECT, "IsBlockValueValid -- Block is not in budget cycle window, checking block value against block reward\n");
        if(!isBlockRewardValueMet) {
    ...
    LogPrint(DYNLog::GOBJECT, "block.vtx[0].GetValueOut() %lld <= nSuperblockMaxValue %lld\n", block.vtx[0].GetValueOut(), nSuperblockMaxValue);
    ...
    if(sporkManager.IsSporkActive(SPORK_9_SUPERBLOCKS_ENABLED)) {
        ...
        if(CSuperblockManager::IsSuperblockTriggered(nBlockHeight)) {
            if(CSuperblockManager::IsValid(block.vtx[0], nBlockHeight, blockReward)) {
                LogPrint(DYNLog::GOBJECT, "IsBlockValueValid -- Valid superblock at height %d: %s", nBlockHeight, block.vtx[0].ToString());
        ...
        LogPrint(DYNLog::GOBJECT, "IsBlockValueValid -- No triggered superblock detected at height %d\n", nBlockHeight);
        if(!isBlockRewardValueMet) {
        ...
        LogPrint(DYNLog::GOBJECT, "IsBlockValueValid -- Superblocks are disabled, no superblocks allowed\n");
        if(!isBlockRewardValueMet) {
...
bool IsBlockPayeeValid(const CTransaction& txNew, int nBlockHeight, CAmount blockReward)
{
    ...
    if(nBlockHeight < consensusParams.nSuperblockStartBlock) {
        if(dnpayments.IsTransactionValid(txNew, nBlockHeight)) {
            LogPrint(DYNLog::DNPAYMENTS, "IsBlockPayeeValid -- Valid Dynode payment at height %d: %s", nBlockHeight, txNew.ToString());
            return true;
        }
        ...
        if(nBlockHeight >= consensusParams.nBudgetPaymentsStartBlock &&
            ...
            LogPrint(DYNLog::GOBJECT, "IsBlockPayeeValid -- WARNING: Probably valid budget block, have no data, accepting\n");
            // TODO: reprocess blocks to make sure they are legit?
            return true;
        }
    ...
    if(!sporkManager.IsSporkActive(SPORK_9_SUPERBLOCKS_ENABLED)) {
        if(CSuperblockManager::IsSuperblockTriggered(nBlockHeight)) {
            if(CSuperblockManager::IsValid(txNew, nBlockHeight, blockReward)) {
                LogPrint(DYNLog::GOBJECT, "IsBlockPayeeValid -- Valid superblock at height %d: %s", nBlockHeight, txNew.ToString());
                return true;
            }
        ...
        LogPrint(DYNLog::GOBJECT, "IsBlockPayeeValid -- No triggered superblock detected at height %d\n", nBlockHeight);
    } else {
        // should NOT allow superblocks at all, when superblocks are disabled
        LogPrint(DYNLog::GOBJECT, "IsBlockPayeeValid -- Superblocks are disabled, no superblocks allowed\n");
    }
    ...
    if (strCommand == NetMsgType::DYNODEPAYMENTSYNC) { //Dynode Payments Request Sync
...

        {
            ...
            if(mapDynodePaymentVotes.count(nHash)) {
                LogPrint(DYNLog::DNPAYMENTS, "DYNODEPAYMENTVOTE -- hash=%s, nHeight=%d seen\n", nHash.ToString(), pCurrentBlockIndex->nHeight);
                return;
            }
        ...
        if(vote.nBlockHeight < nFirstBlock || vote.nBlockHeight > pCurrentBlockIndex->nHeight+20) {
            LogPrint(DYNLog::DNPAYMENTS, "DYNODEPAYMENTVOTE -- vote out of range: nFirstBlock=%d, nBlockHeight=%d, nHeight=%d\n", nFirstBlock, vote.nBlockHeight, pCurrentBlockIndex->nHeight);
            return;
        }

        std::string strError = "";
        if(!vote.IsValid(pfrom, pCurrentBlockIndex->nHeight, strError)) {
            LogPrint(DYNLog::DNPAYMENTS, "DYNODEPAYMENTVOTE -- invalid message, error: %s\n", strError);
            return;
        }
        ...
        if(!vote.CheckSignature(dnInfo.pubKeyDynode, pCurrentBlockIndex->nHeight, nDos)) {
            if(nDos) {
                ...
                LogPrint(DYNLog::DNPAYMENTS, "DYNODEPAYMENTVOTE -- WARNING: invalid signature\n");
            }
...
bool CDynodeBlockPayees::GetBestPayee(CScript& payeeRet)
{
    ...
    if(!vecPayees.size()) {
        LogPrint(DYNLog::DNPAYMENTS, "CDynodeBlockPayees::GetBestPayee -- ERROR: couldn't find any payee\n");
        return false;
    }
...
bool CDynodeBlockPayees::HasPayeeWithVotes(CScript payeeIn, int nVotesReq)
{
    ...
    LogPrint(DYNLog::DNPAYMENTS, "CDynodeBlockPayees::HasPayeeWithVotes -- ERROR: couldn't find any payee with %d+ votes\n", nVotesReq);
    return false;
}
        ...
        if (payee.GetVoteCount() >= DNPAYMENTS_SIGNATURES_REQUIRED) {
            BOOST_FOREACH(CTxOut txout, txNew.vout) {
                if (payee.GetPayee() == txout.scriptPubKey && nDynodePayment == txout.nValue) {
                    LogPrint(DYNLog::DNPAYMENTS, "CDynodeBlockPayees::IsTransactionValid -- Found required payment\n");
                    return true;
                }
            }
...
void CDynodePayments::CheckAndRemove()
{
    ...
    while(it != mapDynodePaymentVotes.end()) {
        ...
        if(pCurrentBlockIndex->nHeight - vote.nBlockHeight > nLimit) {
            LogPrint(DYNLog::DNPAYMENTS, "CDynodePayments::CheckAndRemove -- Removing old Dynode payment: nBlockHeight=%d\n", vote.nBlockHeight);
...
bool CDynodePayments::ProcessBlock(int nBlockHeight)
{
    ...
    if (nRank == -1) {
        LogPrint(DYNLog::DNPAYMENTS, "CDynodePayments::ProcessBlock -- Unknown Dynode\n");
        return false;
    }

    if (nRank > DNPAYMENTS_SIGNATURES_TOTAL) {
        LogPrint(DYNLog::DNPAYMENTS, "CDynodePayments::ProcessBlock -- Dynode not in the top %d (%d)\n", DNPAYMENTS_SIGNATURES_TOTAL, nRank);
        return false;
    }
...
void CDynodePayments::UpdatedBlockTip(const CBlockIndex *pindex)
{
    ...
    LogPrint(DYNLog::DNPAYMENTS, "CDynodePayments::UpdatedBlockTip -- pCurrentBlockIndex->nHeight=%d\n", pCurrentBlockIndex->nHeight);
